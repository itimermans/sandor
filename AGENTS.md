...
After editing Python files (like `sandor/routes.py`):

```bash
python -m py_compile sandor/__init__.py  # Check single file for syntax errors
```
...
- Backend processing in `sandor/routes.py`
...
- **✅ Do**: `sandor/` with underscores, all lowercase
...
export namespace CommandIDs {
  export const openPanel = 'sandor:open-panel';
  export const refreshData = 'sandor:refresh-data';
}
...
- Backend routes: `sandor/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `sandor/` directory
...
1. **Read the backend first** — Check `sandor/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `sandor/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "sandor", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `sandor/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `sandor/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable sandor
   ```
...
- **Output**: Creates files in `lib/` and `sandor/labextension/`
...
**After editing Python** (files in `sandor/`):
...
- Ask user to search for the extension ID (`sandor`) to see if it loaded
...
ls -la sandor/labextension/  # Should contain bundled extension
```
...
- Backend routes: `sandor/routes.py`
- Server extension config: `jupyter-config/server-config/sandor.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `sandor/routes.py`
...
**❌ Don't**: Use absolute imports like `from sandor.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'sandor:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'sandor:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-sandor-ClassName`
- **API routes** (in `sandor/routes.py`): `url_path_join(base_url, "sandor", "endpoint")`
